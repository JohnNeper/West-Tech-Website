...
const slides = [
  { image: heroBg, titleKey: 'hero.slide1.title', subtitleKey: 'hero.slide1.subtitle' },
  { image: aboutImage, titleKey: 'hero.slide2.title', subtitleKey: 'hero.slide2.subtitle' },
  { image: ecosystemImage, titleKey: 'hero.slide3.title', subtitleKey: 'hero.slide3.subtitle' },
];
    ...
    <section className="relative h-screen w-full overflow-hidden">
      {slides.map((s, i) => (
        <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${i === current ? 'opacity-100' : 'opacity-0'}`}>
          <img src={s.image} alt="" className="w-full h-full object-cover scale-105" width={1920} height={1080} />
        </div>
      ))}
      ...
      <div className="relative z-10 flex flex-col justify-center h-full container mx-auto px-6">
        <div className="max-w-3xl">
          <motion.div key={current} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94] }}>
            ...
            <p className="font-body text-sm sm:text-base md:text-lg text-white/70 tracking-wider max-w-xl mb-10 leading-relaxed">
              {t(slide.subtitleKey)}
            </p>
          </motion.div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-4">
        ...
        <div className="flex gap-2">
          {slides.map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`h-[2px] transition-all duration-500 ${i === current ? 'w-10 bg-warm-gold' : 'w-6 bg-white/30'}`} />
          ))}
        </div>
